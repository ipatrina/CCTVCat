...
function cctvGet($h, $c, $d)
{
    $h = curl_init();
    curl_setopt(
        $h,
        CURLOPT_URL,
        "http://api.cntv.cn/epg/getEpgInfoByChannelNew?c=" .
            $c .
            "&serviceId=tvcctv&d=" .
            $d .
            "&t=jsonp&cb=setItem1"
    ...
    $html = curl_exec($h);
    return json_decode(substr($html, 9, strlen($html) - 11), true);
}
...
$date = getWeek();
if ($service == "null") {
    $service = $channelIds[0];
}

$channelMode = !is_numeric($service);
?>
...
if ($channelMode) {
    for ($i = 1; $i <= count($channelIds); $i++) {
        print '<td style="background-color: lightblue; width: 12.5%;"><p><a class="heading" href="' .
            ($channelIds[$i - 1] == "v2"
                ? "../v2/"
    ...
    for ($i = 1; $i <= count($date); $i++) {
        print '<td style="background-color: lightblue;"><p><a class="heading" href="./?Service=' .
            $date[$i] .
            '">' .
            formatDate($date[$i]) .
            "</a></p></td>";
...
function printEpgTable($epgData, $epgChannelName)
{
    ...
    for ($j = 1; $j <= count($epgData); $j++) {
        ...
        print '<td style="width: 350px; text-align: left;"><h4>' .
            $epgChannelName .
            " " .
            date("Y-m-d H:i:s", $epgData[$j - 1]["startTime"]) .
            "</h4></div>";
...
if ($channelMode) {
    for ($i = 1; $i <= count($date); $i++) {
        ...
        if (isset($jsonContext["data"][$service]["list"])) {
            ...
            $epgData = $jsonContext["data"][$service]["list"];
            if (count($epgData) > 0) {
                print '<h3><a class="subHeading" href="./?Service=' .
                    $date[$i] .
                    '">' .
                    formatDate($date[$i]) .
                    "</a></h3>";
                printEpgTable($epgData, $epgChannelName);
            }
        }
    }
} else {
    for ($i = 0; $i < count($channelIds); $i++) {
        $jsonContext = cctvGet($curlHandle, $channelIds[$i], $service);
        $epgChannelName = $jsonContext["data"][$channelIds[$i]]["channelName"];
        $epgData = $jsonContext["data"][$channelIds[$i]]["list"];
        if (!empty($epgData) && count($epgData) > 0) {
            print '<h3><a class="subHeading" href="./?Service=' .
                $channelIds[$i] .
                '">' .
                $epgChannelName .
